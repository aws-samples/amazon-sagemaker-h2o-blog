...
def lambda_handler(event, context):
    ...
    for record in event["Records"]:
        ...
        PARAMETERS["model"]["name"] = "{}-{}".format(
            PARAMETERS["model"]["name"], TS.replace("-", "")
        )
        PARAMETERS["tuningJobName"] = "{}-{}".format(
            PARAMETERS["tuningJobName"], TS.replace("-", "")
        )
        ...
        PARAMETERS["trainingInstanceVolumeSizeInGB"] = int(
            PARAMETERS["trainingInstanceVolumeSizeInGB"]
        )
        ...
        PARAMETERS["targetTrackingScalingPolicyConfiguration"]["ScaleOutCooldown"] = int(
            PARAMETERS["targetTrackingScalingPolicyConfiguration"]["ScaleOutCooldown"]
        )
        PARAMETERS["targetTrackingScalingPolicyConfiguration"]["ScaleInCooldown"] = int(
            PARAMETERS["targetTrackingScalingPolicyConfiguration"]["ScaleInCooldown"]
        )

        PARAMETERS["targetTrackingScalingPolicyConfiguration"]["TargetValue"] = float(
            PARAMETERS["targetTrackingScalingPolicyConfiguration"]["TargetValue"]
        )

        PARAMETERS["enableManagedSpotTraining"] = bool(PARAMETERS["enableManagedSpotTraining"])
        PARAMETERS["targetTrackingScalingPolicyConfiguration"]["DisableScaleIn"] = bool(
            PARAMETERS["targetTrackingScalingPolicyConfiguration"]["DisableScaleIn"]
        )

        print(json.dumps(PARAMETERS))
        SFN_CLIENT.start_execution(stateMachineArn=STATE_MACHINE_ARN, input=json.dumps(PARAMETERS))
