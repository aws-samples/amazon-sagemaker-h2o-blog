...
def lambda_handler(event, context):
    if "inferenceImage" not in event["model"]["hosting"]:
        ...
        if len(algorithm_resp["InferenceSpecification"]["Containers"]) > 0:
            event["model"]["hosting"]["inferenceImage"] = algorithm_resp["InferenceSpecification"][
                "Containers"
            ][0]["Image"]
    ...
    if "acceleratorType" in event["model"]["hosting"].keys():
        event["productionVariants"]["AcceleratorType"] = event["model"]["hosting"][
            "acceleratorType"
        ]

    event["bestModelDataUrl"] = (
        event["model"]["artifactsS3OutputPath"]
        + event["modelTraining"]["BestTrainingJob"]["TrainingJobName"]
        + "/output/model.tar.gz"
    )
